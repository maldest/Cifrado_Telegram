...
# Función para manejar el mensaje de bienvenida automáticamente
async def welcome_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Si el usuario no ha iniciado la conversación con /start, el bot responde automáticamente
    await update.message.reply_text(
        "¡Hola! Soy tu bot de cifrado. Aquí están los comandos disponibles:\n\n"
        "/start - Inicia la conversación\n"
        "/cifrar - Envia un mensaje para cifrar\n"
...
async def descifrar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Por favor, ingresa el mensaje cifrado y el IV (separados por espacio).")
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == 'cifrar':
        ...
        # Guardar el mensaje cifrado y el IV en context.user_data para el descifrado
        context.user_data['encrypted_message'] = mensaje_cifrado
        context.user_data['iv'] = iv
        
        # Responder con el mensaje cifrado y el IV
        await update.message.reply_text(f"Mensaje cifrado: {mensaje_cifrado}\nIV: {iv}")

    elif action == 'descifrar':
        try:
            # El usuario debe ingresar el mensaje cifrado y el IV
            mensaje_cifrado, iv = update.message.text.split()

            # Validar que el mensaje cifrado y el IV estén en formato base64
            if not (base64.b64encode(base64.b64decode(mensaje_cifrado)) == mensaje_cifrado.encode() and
                    base64.b64encode(base64.b64decode(iv)) == iv.encode()):
                await update.message.reply_text("El mensaje cifrado o el IV no están en el formato correcto. Asegúrate de que ambos sean cadenas válidas en base64.")
                return
            
            # Clave de 16 bytes, longitud válida para AES
            clave = b"miclavesecreta12"
...
# Manejador de mensajes para dar la bienvenida cuando el usuario empieza a interactuar
